    ...
    <el-card shadow="never" style="margin-top: 15px">
      <div class="operate-container">
        ...
        <div class="operate-button-container" v-show="order.status===1 ||order.status===2">
          <el-button size="small" @click="handleCompleteOrder">已完成</el-button>
    ...
    <el-dialog title="修改费用信息"
      ...
      <div class="table-layout">
        ...
        <el-row>
          <el-col :span="6" class="table-cell">-￥{{order.promotionAmount}}</el-col>
          <el-col :span="6" class="table-cell">
            <el-input-number v-model="moneyInfo.discountAmount" :precision="2" :step="0.5" size="small" />
          </el-col>
          <el-col :span="6" class="table-cell">
...
<script>
  import {
    getOrderDetail,updateReceiverInfo,updateMoneyInfo,closeOrder,
    updateOrderNote,deleteOrder,cancelOrder,updateOrderCompletedStatus
    } from '@/api/order';
  // import LogisticsDialog from '@/view/orderManage/components/logisticsDialog.vue';
  ...
  export default {
    ...
    created() {
      ...
      getOrderDetail({"id": this.id}).then(response => {
        ...
        this.receiverInfo.receiverName = this.order.receiverName;
        this.receiverInfo.receiverPhone = this.order.receiverPhone;
        this.receiverInfo.receiverPostCode = this.order.receiverPostCode;
        this.receiverInfo.receiverDetailAddress = this.order.receiverDetailAddress;
      });
    ...
    methods: {
      ...
      showCloseOrderDialog(){
        ...
        this.closeInfo.id = Number(this.id);
      },
      handleCloseOrder(){
        this.$confirm('是否要关闭?', '提示', {
          ...
          type: 'warning'
            ...
            let data = {"ids": [this.closeInfo.id], "note": this.closeInfo.note}
            closeOrder(data).then(response=>{
      ...
      handleMarkOrder(){
        this.$confirm('是否要备注订单?', '提示', {
          ...
          let data = {"id": this.markInfo.id, "note": this.markInfo.note, "status": this.order.status}
          updateOrderNote(data).then(response=>{
      ...
      handleDeleteOrder(){
        this.$confirm('是否要进行该删除操作?', '提示', {
          ...
          deleteOrder({"ids": [this.id]}).then(response=>{
            this.$message({
              message: '删除成功！',
              type: 'success',
              duration: 1000
      ...
      handleCompleteOrder() {
        updateOrderCompletedStatus({"ids": [this.id]}).then(response=>{
          this.$message({
            message: '更新成功！',
            type: 'success',
            duration: 1000
      ...
      handleCancelOrder() {
        cancelOrder({"ids": [this.id]}).then(response=>{
          this.$message({
            message: '取消成功！',
            type: 'success',
            duration: 1000
