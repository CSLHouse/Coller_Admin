			...
			initPromotionTipList(data) {
				...
				if (promotionType == 0) {
					...
					for (let item of data.productLadderList) {
						this.promotionTipList.push("满" + item.count + "件打" + item.reducePrice(item.discount) * 10 + "折");
					}
				} else if (promotionType == 4) {
					this.promotionTipList.push("满减优惠");
					for (let item of data.productFullReductionList) {
						this.promotionTipList.push("满" + numFilter(item.fullPrice) + "元减" + numFilter(item.reducePrice) + "元");
					}
