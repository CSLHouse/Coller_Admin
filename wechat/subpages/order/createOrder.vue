<template>
	<view>
		...
		<view style="padding: 30upx 0;background: #fff;">
			请到猪迪克自提
			<text class="yticon icon-you"></text>
		</view>
		...
		<view class="footer">
			...
			<view class="submit">
				<!-- <button type="warn" style="width: 280upx; height: 100%;" :disabled="!currentAddress.id" @click="submit">提交订单</button> -->
				<button type="warn" style="width: 280upx; height: 100%;" @click="submit">提交订单</button>
			</view>
			
			<!-- <text class="submit" disabled={!currentAddress.id} @click="submit">提交订单</text> -->
		</view>
		...
		methods: {
			...
			async loadData() {
				generateConfirmOrder({'ids': this.cartIds}).then(response => {
					...
					this.pickUp = response.data.PickupType
				});
			},
			//显示优惠券面板
			toggleMask(type) {
			...
			submit() {
				...
				if (this.pickUp > 0 && _this.currentAddress.id <= 0) {
